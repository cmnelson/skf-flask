...
if [[ "$HTTPS" == "true" ]]; then
    if [[ -e "/skf-flask/server.pem" && -e "/skf-flask/server.key" ]]; then
        ...
        if [[ ! -z "$CERT" && ! -z "$KEY" ]]; then
            ...
            openssl req -nodes -newkey rsa:4096 -keyout /skf-flask/server.key -out /skf-flask/server.csr  -subj "/CN=$ORIGIN"       
...
if [[ "$ORIGIN" != "" ]]; then
    ...
    perl -pi -e "s/ng serve --host=0.0.0.0 --env=prod/ng serve --public $ORIGIN:443 --env=prod/" /skf-flask/Angular/package.json
else
